...
The core list of dead products is maintained in the [corpses.json](src/WebClient/app/corpses.json) file within the `src/WebClient/app/graveyard/` directory. The document contains an array of objects using a custom JSON schema. The schema is defined in the [corpses.schema.json](/files/corpses.schema.json) file but can be summarized as the following properties:
